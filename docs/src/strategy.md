...
independently owned `ClusterSequence`. This remains the right interface when a
result must be retained.

High-throughput applications that process many independent events can avoid
most per-event temporary allocations with an `N2TiledWorkspace`:
...
for event in events
    with_n2tiled_reconstruction(
        ...
        jets = inclusive_jets!(workspace, clusterseq; ptmin = 5.0)
        # Consume `jets` here, or copy it if it must outlive the callback.
    end
end
```

The callback receives a complete `ClusterSequence`, so history, constituent,
and exclusive-jet queries remain available. Its jets and history, and the
vector returned by `inclusive_jets!`, borrow storage from the workspace. They
are overwritten the next time that workspace is used. Copy values that must
outlive the callback or use the ordinary owning interface.
