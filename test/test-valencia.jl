...
"""
function match_jets_with_pairing_optimization(ref_jets, julia_jets; pt_tolerance=0.5, rap_tolerance=1.0, pt_similarity_threshold=0.01)
    ...
    if n_ref == 2 && n_julia == 2
        ...
        julia_pts = [begin
            jet_pj = PseudoJet(px(julia_jets[i]), py(julia_jets[i]), pz(julia_jets[i]), energy(julia_jets[i]))
            pt(jet_pj)
        end for i in 1:2]
        ...
        if abs(ref_pts[1] - ref_pts[2]) < pt_similarity_threshold
            ...
            jet1_pj = PseudoJet(px(julia_jets[1]), py(julia_jets[1]), pz(julia_jets[1]), energy(julia_jets[1]))
            jet2_pj = PseudoJet(px(julia_jets[2]), py(julia_jets[2]), pz(julia_jets[2]), energy(julia_jets[2]))
    ...
    for (ref_idx, ref_jet) in ref_jets_sorted
        ...
        for (julia_idx, julia_jet) in enumerate(julia_jets)
            ...
            jet_pj = PseudoJet(px(julia_jet), py(julia_jet), pz(julia_jet), energy(julia_jet))
            julia_pt = pt(jet_pj)
...
function match_jets(ref_jets, julia_jets; pt_tolerance=0.5, rap_tolerance=1.0)
    return match_jets_with_pairing_optimization(ref_jets, julia_jets; pt_tolerance=pt_tolerance, rap_tolerance=rap_tolerance)
end
...
@testset "Valencia algorithm basic test" begin
    ...
    if isfile(eventfile)
        ...
        for (evt_idx, event) in enumerate(events)
            ...
            clusterseq = ee_genkt_algorithm(filtered_event, algorithm=JetAlgorithm.Valencia, p=0.8, γ=0.8, R=1.2)
            ...
            if evt_idx == 13
                ...
                matched_pairs = match_jets(inclusive_ref_data, inclusive_20gev; pt_tolerance=0.5, rap_tolerance=2.0)
            ...
            matched_pairs = match_jets(inclusive_ref_data, inclusive_20gev; pt_tolerance=0.1, rap_tolerance=2.0)
            ...
            if length(matched_pairs) == 2
                ...
                if abs(ref_pts[1] - ref_pts[2]) < 0.01
                    ...
                    jet1_pj = PseudoJet(px(matched_pairs[1][2]), py(matched_pairs[1][2]), pz(matched_pairs[1][2]), energy(matched_pairs[1][2]))
                    jet2_pj = PseudoJet(px(matched_pairs[2][2]), py(matched_pairs[2][2]), pz(matched_pairs[2][2]), energy(matched_pairs[2][2]))
                    rap1 = rapidity(jet1_pj)
                    rap2 = rapidity(jet2_pj)
                    ref_rap1 = matched_pairs[1][1]["rap"]
                    ref_rap2 = matched_pairs[2][1]["rap"]
                    # Arrangement 1
                    arrangement1 = abs(rap1 - ref_rap1) < 0.15 && abs(rap2 - ref_rap2) < 0.15
                    # Arrangement 2 (swapped)
                    arrangement2 = abs(rap2 - ref_rap1) < 0.15 && abs(rap1 - ref_rap2) < 0.15
                ...
                for (ref_jet, julia_jet, ref_idx, julia_idx) in matched_pairs
                    ...
                    our_pt = pt(jet_pj)
                    our_rap = rapidity(jet_pj)
                    if !(abs(our_pt - ref_jet["pt"]) < 0.1) || !(abs(our_rap - ref_jet["rap"]) < 0.15)
                        failed = true
                    end
                end
                if failed
            ...
            # Match jets
            matched_pairs = match_jets(exclusive_ref_data, exclusive_4; pt_tolerance=0.1, rap_tolerance=2.0)
            ...
            matched_pairs = match_jets(exclusive_d500_ref_data, exclusive_d500; pt_tolerance=0.1, rap_tolerance=2.0)
