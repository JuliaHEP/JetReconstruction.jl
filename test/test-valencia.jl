...
@testset "Valencia precomputed helpers (unit)" begin
    ...
    @test JetReconstruction.valencia_distance_inv_scaled_arrays(E2p_scaled, nx, ny, nz, i,
                                                                j) ≈
          min(E2p_scaled[i], E2p_scaled[j]) * ang
    ...
    JetReconstruction.get_angular_nearest_neighbours_valencia_precomputed!(eereco,
                                                                           E2p_scaled,
                                                                           beam_term, pwr,
                                                                           γ, R)
    ...
    nnd = copy(eereco.nndist)
    nni = copy(eereco.nni)
    dijd = copy(eereco.dijdist)
    JetReconstruction.update_nn_no_cross_arrays_precomputed!(nnd, nni, nx, ny, nz,
                                                             E2p_scaled, beam_term, dijd, 2,
                                                             N, 1.0, pwr, γ, R)
    @test isfinite(nnd[2])
    ...
    nni2 = copy(nni)
    dijd2 = copy(dijd)
    JetReconstruction.update_nn_cross_arrays_precomputed!(nnd2, nni2, nx, ny, nz,
                                                          E2p_scaled, beam_term, dijd2, 2,
                                                          N, 1.0, pwr, γ, R)
