...
function compare_results(
        ptr::Ptr{C_JetReconstruction.C_ClusterSequence{T}},
        cluster_seq::ClusterSequence{T}
    ) where {T}
    @test ptr != C_NULL
    ...
    c_history = unsafe_wrap(
        Vector{JetReconstruction.HistoryElement}, c_cluster_seq.history,
        c_cluster_seq.history_length
    )
    @test all(struct_approx_equal.(c_history, cluster_seq.history))
    return @test c_cluster_seq.Qtot ≈ cluster_seq.Qtot
end

function compare_results(
        ptr::Ptr{C_JetReconstruction.C_JetsResult{T}},
        jets::Vector{T}
    ) where {T}
    @test ptr != C_NULL
    ...
    return @test all(struct_approx_equal.(c_data, jets))
end
...
function test_pseudojet()
    ...
    return struct_approx_equal(jet, c_jet)
end

function test_jet_reconstruct(
        filename, ::Type{T} = PseudoJet; algorithm, R, strategy,
        recombine, power = 0.0
    ) where {T}
    @testset "C-interface jet reconstruct" begin
        ...
        c_results = Vector{Ptr{C_JetReconstruction.C_ClusterSequence{T}}}(
            undef,
            length(events)
        )
        for (ievent, event) in enumerate(events)
            ...
            ret = C_JetReconstruction.jetreconstruction_jet_reconstruct(
                c_event,
                c_event_length,
                algorithm,
                power,
            ...
            cluster_seq = JetReconstruction.jet_reconstruct(
                event; R = R,
                p = power,
                algorithm = algorithm,
                strategy = strategy,
...
function test_inclusive_jets(
        cluster_seq_ptrs::Vector{Ptr{C_JetReconstruction.C_ClusterSequence{T}}},
        cluster_seqs::Vector{JetReconstruction.ClusterSequence{T}};
        ptmin
    ) where {T}
    return @testset "C-interface inclusive jets" begin
        ...
        for (cluster_seq_ptr, cluster_seq) in zip(cluster_seq_ptrs, cluster_seqs)
            ret = C_JetReconstruction.jetreconstruction_inclusive_jets(
                cluster_seq_ptr,
                ptmin,
                results_ptr
            )
...
function test_exclusive_jets_njets(
        cluster_seq_ptrs::Vector{Ptr{C_JetReconstruction.C_ClusterSequence{T}}},
        cluster_seqs::Vector{JetReconstruction.ClusterSequence{T}};
        njets
    ) where {T}
    return @testset "C-interface exclusive jets njets" begin
        results_ptr = Ptr{C_JetReconstruction.C_JetsResult{T}}(Libc.malloc(sizeof(C_JetReconstruction.C_JetsResult{T})))
        for (cluster_seq_ptr, cluster_seq) in zip(cluster_seq_ptrs, cluster_seqs)
            ret = C_JetReconstruction.jetreconstruction_exclusive_jets_njets(
                cluster_seq_ptr,
                Csize_t(njets),
                results_ptr
            )
...
function test_exclusive_jets_dcut(
        cluster_seq_ptrs::Vector{Ptr{C_JetReconstruction.C_ClusterSequence{T}}},
        cluster_seqs::Vector{JetReconstruction.ClusterSequence{T}};
        dcut
    ) where {T}
    return @testset "C-interface exclusive jets dcut" begin
        results_ptr = Ptr{C_JetReconstruction.C_JetsResult{T}}(Libc.malloc(sizeof(C_JetReconstruction.C_JetsResult{T})))
        for (cluster_seq_ptr, cluster_seq) in zip(cluster_seq_ptrs, cluster_seqs)
            ret = C_JetReconstruction.jetreconstruction_exclusive_jets_dcut(
                cluster_seq_ptr,
                dcut,
                results_ptr
            )
...
@testset "C-interface JetReconstruction pp" begin
    ...
    for alg in [JetAlgorithm.AntiKt, JetAlgorithm.CA, JetAlgorithm.Kt, JetAlgorithm.GenKt],
        ...
        @testset "C-interface JetReconstruction comparison: alg=$alg, R=$test_cone_size, strategy=$stg" begin
            cluster_seq_ptrs, cluster_seqs = test_jet_reconstruct(
                events_file_pp;
                algorithm = alg,
                strategy = stg,
