...
@testset "Common jet utilities" begin
    ...
    lvhep_kt_scale = min(JetReconstruction.pt(pj1), JetReconstruction.pt(pj2)) *
        deltar(
        JetReconstruction.lorentzvector_cyl(pj1),
    ...
    pj1_ptyetaphi = PseudoJet(;
        pt = JetReconstruction.pt(pj1),
    ...
    eej1_ptyetaphi = EEJet(;
        pt = JetReconstruction.pt(eej1),
...
@testset "Final jets extraction" begin
    ...
    vec_lorentzhep = [
        JetReconstruction.lorentzvector(pj1),
