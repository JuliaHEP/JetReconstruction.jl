    ...
    @testset "Jet Flavour Constituent Utilities" begin
        ...
        if !isfile(reference_file)
            ...
            if !isfile(edm4hep_example_file)
                ...
                jet_constituents = ext_mod.build_constituents_cluster(recps,
                                                                      constituent_indices)
                            ...
                            for i in 1:length(actual)
                                ...
                                for j in 1:length(actual[i])
                                    ...
                                    elseif isinf(ref_val)
                                        @test isinf(act_val) &&
                                              sign(act_val) == sign(ref_val)
                                    else
                                        @test isapprox(act_val, ref_val; atol = atol,
                                                       rtol = rtol)
                                    end
                                end
                            end
                ...
                @testset "Particle Identification" begin
                    ...
                    compare_outputs(helper_mod.get_is_charged_had(jet_constituents),
                                    reference_outputs["get_is_charged_had"],
                                    "get_is_charged_had")
                ...
                @testset "Special Measurements" begin
                    ...
                    compare_outputs(helper_mod.get_dndx(jet_constituents, dNdx, trackdata,
                                                        is_charged_had),
                                    reference_outputs["get_dndx"], "get_dndx")
                    compare_outputs(helper_mod.get_mtof(jet_constituents, track_L,
                                                        trackdata, trackerhits,
                ...
                @testset "Impact Parameters and Jet Distance" begin
                    ...
                    compare_outputs(helper_mod.get_btagSip2dSig(btag_sip2d_val, err2_D0),
                                    reference_outputs["get_btagSip2dSig"],
                                    "get_btagSip2dSig")
                    ...
                    compare_outputs(helper_mod.get_btagSip3dSig(btag_sip3d_val, err2_D0,
                                                                err2_Z0),
                                    reference_outputs["get_btagSip3dSig"],
                                    "get_btagSip3dSig")

                    # Jet distance
                    jet_dist_val = helper_mod.get_JetDistVal_clusterV(jets,
                                                                      jet_constituents, D0,
                                                                      Z0, phi0, bz)
                    compare_outputs(jet_dist_val,
                                    reference_outputs["get_JetDistVal_clusterV"],
                                    "get_JetDistVal_clusterV")

                    btag_jet_dist_val = helper_mod.get_btagJetDistVal(jets,
                                                                      jet_constituents, D0,
                                                                      Z0, phi0, bz)
                    compare_outputs(btag_jet_dist_val,
                                    reference_outputs["get_btagJetDistVal"],
                                    "get_btagJetDistVal")

                    compare_outputs(helper_mod.get_JetDistSig(jet_dist_val, err2_D0,
                                                              err2_Z0),
                                    reference_outputs["get_JetDistSig"], "get_JetDistSig")
                    compare_outputs(helper_mod.get_btagJetDistSig(btag_jet_dist_val,
                                                                  err2_D0, err2_Z0),
                                    reference_outputs["get_btagJetDistSig"],
                                    "get_btagJetDistSig")
                end
            end
        end
