...
function main()
    ...
    @testset "Jet Reconstruction Examples" begin
        ...
        @test success(run(pipeline(`julia --project=$example_dir $example_dir/jetreco.jl -S N2Plain -p 1 -R 1.0 $events_file`, devnull)))
        @test success(run(pipeline(`julia --project=$example_dir $example_dir/jetreco.jl -S N2Plain -p 1.5 -A GenKt -R 1.0 $events_file`, devnull)))
    end
end
