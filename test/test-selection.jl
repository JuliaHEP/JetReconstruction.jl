...
function test_exclusive_jets_power(event, algo; p = nothing, should_throw::Bool)
    ...
    return @testset "Exclusive jets selection: alg=$(algo) p=$(p)" begin
        cluster_seq = JetReconstruction.jet_reconstruct(
            event; algorithm = algo, p = p,
            R = test_cone_size,
...
@testset "Exclusive jets selection algorithm power requirements" begin
    ...
    # Tests for pp_event
    test_exclusive_jets_power(
        pp_event, JetAlgorithm.GenKt; p = -1.0,
        should_throw = true
    )
    test_exclusive_jets_power(
        pp_event, JetAlgorithm.GenKt; p = 1.0,
    ...
    # Tests for ee_event
    test_exclusive_jets_power(
        ee_event, JetAlgorithm.EEKt; p = -1.0,
        should_throw = true
    )
    test_exclusive_jets_power(
        ee_event, JetAlgorithm.EEKt; p = 1.0,
