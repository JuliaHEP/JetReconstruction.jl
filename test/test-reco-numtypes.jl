...
for TestType in [Float32, Float16]
    let inputs_ee = JetReconstruction.read_final_state_particles(events_file_ee, EEJet{TestType}; maxevents = 1)
        @testset "EE jet reconstruction with $TestType" begin
            ...
            @test typeof(jet_reconstruct(inputs_ee[1]; algorithm = JetAlgorithm.EEKt, p = -1,
                                        R = 1.0)) == ClusterSequence{TestType, EEJet{TestType}}
        end
    end
    let inputs_pp = JetReconstruction.read_final_state_particles(events_file_pp, PseudoJet{TestType}; maxevents = 1)
