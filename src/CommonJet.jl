"""
    abstract type FourMomentum{T <: Real}

Interface for composite types that includes fields `px`, `py`, `pz`, and `E`
representing the components of a four-momentum vector. All concrete jets used
in the package are subtypes of this type.
...
import Base.eltype
"""
    eltype(::FourMomentum{T})

Return the parameterised type, `T` of the four momentum.
"""
eltype(::FourMomentum{T}) where {T} = T
...
Return the transverse momentum of the four-momentum vector of `j`.
"""
...
"""
    CosTheta(j::FourMomentum)

Compute the cosine of the angle between the momentum vector of `j` and the z-axis.
...
import Base.+;
"""
    +(jet1::J, jet2::J) where {T <: Real, J <: FourMomentum{T}}

Adds two four-momentum vectors together, returning a new jet of the same type.
...
history.*
"""
function +(jet1::J, jet2::J) where {T <: Real, J <: FourMomentum{T}}
    J(jet1.px + jet2.px, jet1.py + jet2.py, jet1.pz + jet2.pz, jet1.E + jet2.E)
end

"""
    addjets(jet1::J, jet2::J; cluster_hist_index::Int = 0) where {T <: Real, J <: FourMomentum{T}}

Add jets' four momenta together, returning a new jet of type `J` with the
...
"""
function addjets(jet1::J, jet2::J;
                 cluster_hist_index::Int = 0) where {T <: Real, J <: FourMomentum{T}}
    J(px(jet1) + px(jet2), py(jet1) + py(jet2), pz(jet1) + pz(jet2),
...
"""
    addjets_ptscheme(jet1::J, jet2::J; cluster_hist_index::Int = 0) where {T <: Real, J <: FourMomentum{T}}

Use the massless ``p_T`` scheme for combining two jets, setting the appropriate
cluster history index for the new jet.
"""
function addjets_ptscheme(jet1::J, jet2::J;
                          cluster_hist_index::Int = 0) where {T <: Real,
                                                              J <: FourMomentum{T}}
    scale1 = pt(jet1)
    scale2 = pt(jet2)
    _addjets_with_scale(scale1, scale2, jet1, jet2, cluster_hist_index)::J
end

"""
    preprocess_escheme(jet::J, ::Type{R};
                                cluster_hist_index::Int = 0) -> R where {T <: Real,
                                                                         J,
                                                                         R <: FourMomentum{T}}

Jet preprocessor for the E-scheme, simply copying the four-momentum and assigning the cluster
history index. Note that `J` can be any type that is convertible to `R`.
"""
function preprocess_escheme(jet::J, ::Type{R};
                            cluster_hist_index::Int = 0) where {T <: Real,
                                                                J,
                                                                R <: FourMomentum{T}}
    R(jet; cluster_hist_index = cluster_hist_index)
end

"""
    preprocess_ptscheme(jet::J, ::Type{R};
                             cluster_hist_index::Int = 0) -> R where {T <: Real,
                                                                      J,
                                                                      R <: FourMomentum{T}}

Jet preprocessor for the massless ``p_T`` schemes, resetting the energy of the
jet to be equal to the 3-momentum of the input jet. Note that `J` can be any
type that is convertible to `R`.
"""
function preprocess_ptscheme(jet::J, ::Type{R};
                             cluster_hist_index::Int = 0) where {T <: Real,
                                                                 J,
                                                                 R <: FourMomentum{T}}
    R(px(jet), py(jet), pz(jet), p(jet); cluster_hist_index = cluster_hist_index)
end

"""
    preprocess_ptscheme(particle::Union{LorentzVector, LorentzVectorCyl},
                        ::Type{R};
                        cluster_hist_index::Int = 0) where {T <: Real, R <: FourMomentum{T}}
...
"""
    addjets_pt2scheme(jet1::J, jet2::J; cluster_hist_index::Int = 0) where {T <: Real, J <: FourMomentum{T}}

Use the massless ``p_T^2`` scheme for combining two jets, setting the
appropriate cluster history index for the new jet.
"""
function addjets_pt2scheme(jet1::J, jet2::J;
                           cluster_hist_index::Int = 0) where {T <: Real,
                                                               J <: FourMomentum{T}}
    scale1 = pt2(jet1)
    scale2 = pt2(jet2)
...
"""
    _addjets_with_scale(scale1, scale2, jet1::J, jet2::J, cluster_hist_index::Int) where {T <: Real, J <: FourMomentum{T}}
...
"""
function _addjets_with_scale(scale1, scale2, jet1::J, jet2::J,
                             cluster_hist_index::Int) where {T <: Real, J <:
                                                                        FourMomentum{T}}
    ...
    J(pt = new_pt, rap = new_rap, phi = new_phi; cluster_hist_index)
end
