...
"""
function filter_outliers_iqr(
        trial_timing::AbstractVector{<:Real}, stats;
        outlier_band::Real = 2.0
    )
    min_val = stats.q25 - outlier_band * stats.iqr
    max_val = stats.q75 + outlier_band * stats.iqr
    return trial_timing[(time -> min_val <= time <= max_val).(trial_timing)]
end
...
function pprint_trial_stats(stats)
    return " - average time per event " * @sprintf("%.2f", stats.mean) * " ± " *
...
function print_statistics(
        trial_timing::AbstractVector{<:Real};
    ...
    return (full = stats, outlier_excluded = filtered_stats)
end
